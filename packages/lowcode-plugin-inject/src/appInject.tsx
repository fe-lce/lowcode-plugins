  ...
  if (!injectedPluginConfigMap) {
    ...
    if (injectedPlugins && injectedPlugins.length > 0) {
      injectedPlugins.forEach((item: any) => {
        ...
        injectedPluginConfigMap[config.name || item.module.pluginName] =
          item.module;
      });
    }
  }
...
export function AppInject(ctx: IPublicModelPluginContext, options: any) {
  ...
  const getInjectPlugin = async (
    ...
    const isGlobal =
      ctx?.registerLevel === IPublicEnumPluginRegisterLevel.Workspace;
    ...
    if (isGlobal) {
      if (injectConfig.has(pluginName, "global")) {
        return injectConfig.get(pluginName, "global")
          ? injectedSameNamePlugin
          : plugin;
      }
    ...
    if (injectConfig.has(pluginName, resourceName, viewName)) {
      return injectConfig.get(pluginName, resourceName, viewName)
        ? injectedSameNamePlugin
        : plugin;
    }
  ...
  ctx.config.set(
    ...
    async (originPlugin: any, ctx: IPublicModelPluginContext, options) => {
      const injectedSameNamePlugin = await getInjectPlugin(
        originPlugin,
        options,
        ctx
      );
    ...
    async init() {
      ...
      workspace.onChangeActiveWindow(async () => {
        for (const pluginName in injectedPluginConfigMap) {
          const injectedSameNamePlugin = await getInjectedPlugin(
            pluginName,
            ctx
          );
          if (!injectedSameNamePlugin) {
          ...
          if (
            injectConfig.get(pluginName, resourceName, viewName) &&
            !currentEditorView?.plugins.has(pluginName)
          ) {
            await currentEditorView?.plugins.register(
              injectedPluginConfigMap[pluginName],
              {
                autoInit: true,
              }
            );
          ...
          if (
            injectConfig.get(pluginName, "global") &&
            !workspace.plugins.has(pluginName)
          ) {
            await workspace.plugins.register(
              injectedPluginConfigMap[pluginName],
              {
                autoInit: true,
              }
        ...
        if (
          workspace.window?.currentEditorView &&
          !workspace.window?.currentEditorView.plugins?.has(
            subPlugin.pluginName
          )
        ) {
