...
export function Pane(props: {
  ...
  const updateConfig = React.useCallback((config) => {
    const newDataSource = Object.entries(config)
      .map(([key, value]) => {
        return {
          pluginName: key,
        };
  ...
  const { injectedSetters } = injectInfo;
  ...
  return (
    <div className="inject-setting-panel">
      <span className="inject-setting-panel-title">设计器插件：</span>
      ...
      <Table className="inject-setting-panel-table" dataSource={dataSource}>
        ...
        <Table.Column
          title="调试注册视图"
          dataIndex="pluginName"
          cell={(pluginName, index, record) => (
            <>
              {options.map((resource) => {
                if (resource.children) {
                  return (
                    <div>
                      <span className="inject-setting-panel-label">
                        {resource.label}
                      </span>
                      {resource.children.map((view) => (
                        <Checkbox
                          checked={props.injectConfig.get(
                            pluginName,
                            resource.value,
                            view.value
                          )}
                          onChange={(checked: boolean) => {
                            props.updateInjectConfig(
                              record.pluginName,
                              resource.value,
                              view.value,
                              checked
      ...
      <InjectItem title="Setter" injectItems={injectedSetters} />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="inject-setting-panel-inject-item">
        {props.injectItems &&
          props.injectItems.map((item) => <span>{item.name}</span>)}
      </div>
    </div>
  );
}
