...
import { buildComponents } from "@felce/lowcode-utils";
...
export const needInject =
  searchParams.get("__injectFrom") || // 历史兼容
...
function getInjectUrls(resourceType, type = "url"): Promise<any> {
  const filter = (_urls) => {
    ...
    return type === "url"
      ? filteredUrls.map((item) => item.url || item)
      : filteredUrls;
  };
  ...
  return new Promise((resolve) => {
    if (!urlCache) {
      ...
      if (
        type === "auto" ||
        urlParams[injectTypeFlag] === "auto" ||
        urlParams[debugFlag] !== undefined
      ) {
        fetchJsonp(`http://${injectServerHost}:8899/apis/injectInfo`)
          .then((res) => res.json())
          .then((data) => {
...
function loadScript(url, callback) {
  const src = ((_url) => {
    const isInFileProtocol =
      _url.indexOf("//") === 0 && window.location.protocol === "file:";
...
function promiseLoadScript(url) {
  return new Promise((rs, rj) => {
    ...
    () => {
      console.info(
        `%c Injected ${url}`,
        "font-weight:bold; font-size: 20px; color: orange;"
      );
    },
    (e) => {
      console.error(e);
...
function envFilter(injects) {
  ...
  const env =
    window.injectEnv ||
    urlParams[injectEnvFlag] ||
    (window.VisualEngine || window.LowcodeEditor || window.AliLowCodeEngine
      ? "design"
      : "preview") ||
    "design";

  let device =
    urlParams[injectDeviceFlag] ||
    (window.g_config && window.g_config.device) ||
    (window.pageConfig && window.pageConfig.device) ||
    "web";
  if (device === "both") {
  ...
  return injects.filter((item) => {
    if (env === "design") {
      ...
      if (item.type === "prototype") {
        ...
        if (!item.subType && prototypeKey) {
          ...
          const proto = injects.find(
            (item2) =>
              item2.packageName === item.packageName &&
              item2.type === "prototype" &&
              item2.subType === prototypeKey
          );
          if (proto) {
            return false;
          }
    ...
    if (env === "preview") {
      ...
      if (
        ...
        const viewMobile = injects.find(
          (item2) =>
            item2.packageName === item.packageName &&
            item2.type === "view" &&
            item2.subType === "mobile"
        );
        if (viewMobile) {
          return false;
        }
...
export async function injectComponents(components) {
  ...
  const injectedComponentsForRenderer = await buildComponents(
    libraryMap,
    componentsMap,
    undefined
  );
